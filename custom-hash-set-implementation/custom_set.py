...
class MySet():
    ...
    def add(self, key):
        ...
        for step in range(self._capacity):
            ...
            if slot is None and first_deleted is None: # insert
                self._table[idx] = Entry(key, hash_)
                self._size += 1
                return
            elif slot is self._DELETED and first_deleted is None:
                first_deleted = idx
            elif slot is not None and slot._key == key: # already inside the table, do noting
                return
        ...
        self._table[first_deleted] = Entry(key, hash_)
        self._size += 1
    ...
    def remove(self, key):
        ...
        if index is not None:
            ...
            raise KeyError(f"{key} not found in the set")
    ...
    def __repr__(self):
        return f"MySet({{{', '.join(map(str, list(self)))}}})"
